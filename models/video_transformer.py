...
def get_vit_base_patch16_224(**kwargs):
    vit = TimeSformer(num_frames=kwargs['num_frames'], pretrain_pth=kwargs['pretrain_pth'],
                      weights_from=kwargs['weights_from'],
                      img_size=kwargs['img_size'], attention_type=kwargs['attention_type'], patch_size=16,
                      embed_dims=768, num_heads=12,
                      in_channels=3, num_transformer_layers=12, conv_type='Conv2d', dropout_p=0.,
                      norm_layer=nn.LayerNorm,
...
class MaskFeat(nn.Module):
    ...
    def forward(self, x, target_x, mask, cube_marker, visualize=False):
        ...
        for i, mark_item in enumerate(cube_marker):  # loop for batch
            for marker in mark_item:  # loop for video sample
                ...
                center_index[
                    start_frame * self.stride[0] + span_frame * self.stride[0] // 2] = 1  # center index extends to 16
        ...
        if visualize:
            ...
            mask_preds = rearrange(mask_preds, 'b t h w (dh dw c o) -> b t (h dh) (w dw) c o', dh=2, dw=2, c=3,
                                   o=9)  # need to unnormalize
    ...
    def forward_features(self, x, mask=None):
        ...
        if mask is not None:
            ...
            dense_mask = repeat(mask, 'b t h w -> b t (h dh) (w dw)', dh=self.downsample_rate,
                                dw=self.downsample_rate)  # nearest-neighbor resize
